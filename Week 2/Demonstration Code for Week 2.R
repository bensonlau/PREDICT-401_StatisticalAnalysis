# Week 2 Demonstration Code for PREDICT 401 
...
y <- factor(mag)
summary(y)
sort(summary(y),decreasing=TRUE)
names(summary(y))
...
#variability. It is the ratio of the standard deviation to 
#the mean (average). For example, the expression “The 
#standard deviation is 15% of the mean” is a CV.
#http://www.statisticshowto.com/how-to-find-a-coefficient-of-variation/

100*sd(mag)/mean(mag)

# Find the Winsorized sample standard deviation and check.  Keep in mind the
# psych package estimates quantiles differently.
...
# deviation.  This corresponds to what is shown in Business Statistics.
